namespace dotNet_08_Dateien_01_Dateisuche
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            String endString = "EOF"; // legt die Zeichenfolge fest mit der die Eingabe beendet wird
            List<String> eingabeZeilen;

            if (dateipfad != "-1") // Wenn die Datei gefunden wurde
            {
                Console.WriteLine($"{dateipfad}\nGeben Sie Text ein und Beenden Sie Ihre Eingabe mit {endString}.\n");
        ...
        {
            return Suche(@"..\..\..\", dateiname); // Aufruf der rekursiven Funktion
        }

        static String Suche(String pfad, String dateiname)
        {
            String[] ordnerInPfad = default;
            if (File.Exists($"{pfad}{dateiname}")) return $"{Path.GetFullPath(pfad)}{dateiname}"; // wenn die Datei im aktuellen Pfad gefunden wurde, gib den Pfad zurück
            else
            {
